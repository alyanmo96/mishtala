...
<html>
    ...
    <body>
        ...
        <div class="container">  
        <form id="contact" action="/upload_work_photo" method="post" enctype="multipart/form-data">
            @csrf
            <h4>הוספת תמונה חדשה של עבודת גינון </h4>
            <div class="form-group mt-3">
                <label class="mr-2"></label>
                <input type="file" name="image" required>
            <fieldset>
                <input placeholder="כמה מילים" type="text" name="description"> 
            </fieldset>
            </div>
                    ...
                    @foreach($photo as $i)
                        <div class="col-md-4">
                            <div class="card mb-4 shadow-sm">
                                <img src="{{asset('uploads/work_photo/'.$i->image)}}" class="planets_img">
                                <p>{{$i->description}}</p>
                                <div class="card-body">
                                    <div class="d-flex justify-content-between align-items-center">
                                        <div class="btn-group">                                        
                                            <a href="{{url('/remove_photo/'.$i->id)}}" class="btn btn-sm btn-outline-danger">מחיקה</a>
                                        </div>
                                    </div>
                                </div>
                            </div>
